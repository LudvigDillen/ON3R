...
from .cambridge_pipeline import CambridgeTuplesPipeline
from gluefactory.utils.misc import start_debug

logger = logging.getLogger(__name__)

set_dataset("cambridge")  # Set the default dataset to Cambridge
...
python -m gluefactory.eval.cambridge_absolute_tuples \
    --conf gluefactory/configs/superpoint+lightglue_megadepth_tuples.yaml \
    --checkpoint assets/lightglue_checkpoint.tar eval.estimator=poselib eval.ransac_th=-1 \
    -b star_topology_300_2tuples --overwrite -d

    or     --conf /home2/lu2277di/data/aim/outputs/training/sp+lg_megadepth/20250718_141531_id867/config.yaml \

nohup python -m gluefactory.eval.cambridge_absolute_tuples \
    --conf gluefactory/configs/superpoint+lightglue_megadepth_tuples.yaml \
    --checkpoint assets/lightglue_checkpoint.tar eval.estimator=poselib eval.ransac_th=-1 \
    -b star_topology_300_4tuples --overwrite > outputs/nohups/cambridge_ace_4tuples.txt  2>&1 &
"""
if __name__ == "__main__":
    ...
    CambridgeTuplesPipeline.default_conf["data"]["pairs"] = \
        "assets/cambridge/test_tuples/tuples" + id + ".txt"

    # mingle paths
    output_dir = Path(EVAL_PATH, dataset_name)
    output_dir.mkdir(exist_ok=True, parents=True)

    default_conf = OmegaConf.create(CambridgeTuplesPipeline.default_conf)
    ...
    pipeline = CambridgeTuplesPipeline(conf)
