...
class BaseDataset(metaclass=ABCMeta):
    ...
    base_default_conf = {
        ...
        "prefetch_factor": 2,
        "tuples": "???",
        "tuple_length": 2,  # default tuple length
    }
    default_conf = {}
    ...
    def get_data_loader(self, split, shuffle=None, pinned=False, distributed=False, sample_tuples=False):
        """Return a data loader for a given split."""
        assert split in ["train", "val", "test"]
        dataset = self.get_dataset(split, sample_tuples=sample_tuples)
        try:
