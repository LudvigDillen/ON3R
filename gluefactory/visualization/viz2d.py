...
def plot_matches(kpts0, kpts1, color=None, lw=1.5, ps=4, a=1.0, labels=None, axes=None):
    ...
    Args:
        ...
        a: alpha opacity for the match lines. Can be a scalar or an array-like of shape (N,).
    """
    fig = plt.gcf()
    if axes is None:
        ax = fig.axes
    ...
    # Adapt alpha values: if 'a' is scalar, replicate it for each match.
    if np.isscalar(a):
        a_values = [a] * len(kpts0)
    else:
        a_values = a
