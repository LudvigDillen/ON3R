    ...
    for data_ in tqdm(loader):
        ...
        if tuple_mode:
            for key in data.keys():
                output_file = inner_export_predictions(
                    pred[key], data[key], output_file, as_half, keys, callback_fn, optional_keys,
                    hfile)
        else:
            output_file = inner_export_predictions(
                pred, data, output_file, as_half, keys, callback_fn, optional_keys, hfile)

    hfile.close()
    return output_file


def inner_export_predictions(
    pred,
    data,
    output_file,
    as_half=False,
    ...
    # renormalization
    for k in pred.keys():  # TODO: Probably need an outer loop to iterate over the keys
    ...
    name = data["name"][0]
    if "unique_identifier" in data.keys():
        name = name + data["unique_identifier"][0]
